...
export default class AuthStore {
  rootStore: RootStore;
  user: User | null = { id: 1, name: 'Nathan', email: 'nathan@email.com' };
  ...
  authenticate = async (payload: {
    ...
    try {
      ...
      setCookie(undefined, 'podjs.token', data.access_token, {
  ...
  unauthenticate = () => {
    ...
    destroyCookie(undefined, 'podjs.token');
    this.reset();
  };
  ...
  init = () => {
    const { 'podjs.token': token } = parseCookies();
